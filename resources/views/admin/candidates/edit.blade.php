...
@section('title', 'Edit Candidate - ' . $candidate->candidate_name)
...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8"
    ...
    <nav class="flex text-xs text-slate-500 mb-4" aria-label="Breadcrumb">
        ...
        <span class="mx-2">/</span>
        <span class="text-teal-700 font-semibold">Edit Candidate</span>
    </nav>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {{-- Header Banner --}}
        <div class="bg-gradient-to-r from-slate-900 via-slate-800 to-teal-950 px-6 py-5 text-white">
            <h1 class="text-xl font-bold">Edit Candidate: {{ $candidate->candidate_name }}</h1>
            <p class="text-xs text-teal-200 mt-0.5">Update nominee credentials and dossier details following the standardized 9-field candidate structure.</p>
        </div>

        {{-- Form --}}
        <form method="POST" action="{{ route('admin.candidates.update', $candidate) }}" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-7">
            @csrf
            @method('PUT')
            ...
            <div class="p-5 bg-slate-50/80 rounded-2xl border border-slate-200">
                <label class="block text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">
                    1. Picture <span class="text-slate-400 font-normal lowercase">(optional replacement)</span>
                </label>
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                    ...
                    <div class="w-32 h-40 rounded-xl overflow-hidden bg-slate-200 border-2 border-dashed border-slate-300 flex items-center justify-center flex-shrink-0 shadow-2xs relative">
                        <template x-if="imagePreview">
                            <img :src="imagePreview" alt="New Preview" class="w-full h-full object-cover object-top">
                        </template>
                        <template x-if="!imagePreview">
                            @if($candidate->photo_url)
                                <img src="{{ $candidate->photo_url }}" alt="{{ $candidate->candidate_name }}" class="w-full h-full object-cover object-top">
                            @else
                                <div class="flex flex-col items-center justify-center text-center p-2 text-slate-400">
                                    ...
                                    <span class="text-[10px] font-semibold uppercase tracking-wider">No Photo</span>
                                </div>
                            @endif
                        </template>
                    </div>
                    ...
                    <div class="flex-1 space-y-2">
                        ...
                        <div class="text-[11px] text-slate-500 space-y-0.5">
                            ...
                            <p class="text-slate-400">Select a new image file to replace the existing photo.</p>
                        </div>
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       id="candidate_title" 
                       required 
                       value="{{ old('candidate_title', $candidate->candidate_title) }}" 
            ...
            <div>
                ...
                <select name="discipline_id" 
                    ...
                    @foreach($disciplines as $disc)
                        <option value="{{ $disc->id }}" {{ (string)old('discipline_id', $candidate->discipline_id) === (string)$disc->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       required 
                       value="{{ old('affiliation_to_asm', $candidate->affiliation_to_asm ?? 'ASM Fellow Nominee') }}" 
            ...
            <div>
                ...
                <div class="relative">
                    <input type="url" 
                           ...
                           value="{{ old('nomination_form_url', $candidate->nomination_form_url) }}" 
            ...
            <div>
                ...
                <textarea name="area_of_expertise" 
                          ...
                          class="w-full p-3.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 @error('area_of_expertise') border-rose-500 @enderror">{{ old('area_of_expertise', $candidate->area_of_expertise) }}</textarea>
            ...
            <div>
                ...
                <textarea name="qualifications_professional_memberships" 
                          ...
                          class="w-full p-3.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 @error('qualifications_professional_memberships') border-rose-500 @enderror">{{ old('qualifications_professional_memberships', $candidate->qualifications_professional_memberships) }}</textarea>
            ...
            <div>
                ...
                <textarea name="basis_of_recommendation" 
                          ...
                          class="w-full p-3.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 @error('basis_of_recommendation') border-rose-500 @enderror">{{ old('basis_of_recommendation', $candidate->basis_of_recommendation) }}</textarea>
            ...
            <div class="pt-6 border-t border-slate-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" 
                               name="organisation" 
                               id="organisation" 
                               value="{{ old('organisation', $candidate->organisation) }}" 
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="display_order" 
                               id="display_order" 
                               value="{{ old('display_order', $candidate->display_order) }}" 
                               class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500">
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-2.5 mt-4">
                    <input type="checkbox" 
                           ...
                           id="active" 
                           value="1" 
                           {{ old('active', $candidate->active) ? 'checked' : '' }} 
