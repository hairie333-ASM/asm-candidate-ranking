...
<div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <nav class="flex text-xs text-slate-500 mb-4" aria-label="Breadcrumb">
        ...
        <span class="mx-2">/</span>
        <span class="text-teal-700 font-semibold">Edit User</span>
    </nav>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="bg-gradient-to-r from-slate-900 to-teal-900 px-6 py-4 text-white">
            <h1 class="text-lg font-bold">Edit User Account: {{ $user->name }}</h1>
            <p class="text-xs text-teal-200 mt-0.5">Modify role, active status, or discipline allocation.</p>
        </div>

        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="p-6 sm:p-8 space-y-5" x-data="{ role: '{{ old('role', $user->role) }}' }">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name', $user->name) }}" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500 @error('name') border-rose-500 @enderror">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" required value="{{ old('email', $user->email) }}" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500 @error('email') border-rose-500 @enderror">
                @error('email') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="password" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Reset Password (Leave blank to keep unchanged)</label>
                <input type="password" name="password" id="password" placeholder="Enter new password if resetting" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500 @error('password') border-rose-500 @enderror">
            ...
            <div x-show="role === 'voting_user'" class="p-4 bg-teal-50/70 rounded-xl border border-teal-200">
                ...
                <select name="discipline_id" id="discipline_id" class="w-full px-3.5 py-2 bg-white border border-teal-300 rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500">
                    ...
                    @foreach($disciplines as $disc)
                        <option value="{{ $disc->id }}" {{ (string)old('discipline_id', $user->discipline_id) === (string)$disc->id ? 'selected' : '' }}>
            ...
            <div class="flex items-center space-x-2 pt-2">
                <input type="checkbox" name="active" id="active" value="1" {{ old('active', $user->active) ? 'checked' : '' }} class="rounded border-slate-300 text-teal-600 focus:ring-teal-500 h-4 w-4">
