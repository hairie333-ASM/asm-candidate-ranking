...
@section('title', 'Edit Ranking Exercise')
...
<div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <nav class="flex text-xs text-slate-500 mb-4" aria-label="Breadcrumb">
        ...
        <span class="mx-2">/</span>
        <span class="text-teal-700 font-semibold">Edit Exercise</span>
    </nav>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="bg-gradient-to-r from-slate-900 to-teal-900 px-6 py-4 text-white">
            <h1 class="text-lg font-bold">Edit Exercise: {{ $exercise->exercise_name }}</h1>
            <p class="text-xs text-teal-200 mt-0.5">Update status, timeline, or instructions.</p>
        </div>

        <form method="POST" action="{{ route('admin.exercises.update', $exercise) }}" class="p-6 sm:p-8 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="exercise_name" id="exercise_name" required value="{{ old('exercise_name', $exercise->exercise_name) }}" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500 @error('exercise_name') border-rose-500 @enderror">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500">{{ old('description', $exercise->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500">
                    <option value="Open" {{ old('status', $exercise->status) === 'Open' ? 'selected' : '' }}>Open (Accepting Ballots)</option>
                    <option value="Draft" {{ old('status', $exercise->status) === 'Draft' ? 'selected' : '' }}>Draft (Inactive)</option>
                    <option value="Closed" {{ old('status', $exercise->status) === 'Closed' ? 'selected' : '' }}>Closed (Submissions Locked)</option>
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="start_datetime" id="start_datetime" value="{{ old('start_datetime', $exercise->start_datetime?->format('Y-m-d\TH:i')) }}" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="end_datetime" id="end_datetime" value="{{ old('end_datetime', $exercise->end_datetime?->format('Y-m-d\TH:i')) }}" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-teal-500 @error('end_datetime') border-rose-500 @enderror">
